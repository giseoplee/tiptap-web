!function() {
  ...
  const remove = curry((item, arr) => (arr.splice(arr.indexOf(item), 1), arr));

  const remove2 = curry((arr, item) => (remove(item, arr), item));
  ...
  const uniqueBy = curry((f, coll) => {
    ...
    return filter(a => go(
      a,
      f,
      evd => evds.has(evd) ? false : evds.add(evd)
    ), coll);
  });
  ...
  const Functional = {
    ...
    unique, uniqueBy,
    findVal, find, none, some, every, findWhere, takeWhile,
